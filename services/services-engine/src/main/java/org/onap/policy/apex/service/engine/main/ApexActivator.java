...
public class ApexActivator {
    ...
    public void initializeForMultiplePolicies() throws ApexActivatorException {
        ...
        try {
            apexParameters = (ApexParameters) apexParametersMap.values().toArray()[0];
            ...
            for (ToscaPolicyIdentifier policyIdent : apexParametersMap.keySet()) {
                ApexParameters apexParams = apexParametersMap.get(policyIdent);
                // Check if a policy model file has been specified
                if (apexParams.getEngineServiceParameters().getPolicyModelFileName() != null) {
                    ...
                    policyModelsMap.put(policyIdent, policyModel);
                }
    ...
    private AxPolicyModel aggregatePolicyModels(Map<ToscaPolicyIdentifier, AxPolicyModel> policyModelsMap) {
        ...
        for (ToscaPolicyIdentifier policyIdent: policyModelsMap.keySet()) {
            AxPolicyModel policyModel = policyModelsMap.get(policyIdent);
            String errorMessage = "";
            for (AxArtifactKey key : policyModel.getSchemas().getSchemasMap().keySet()) {
                if (axContextSchemas.getSchemasMap().containsKey(key)) {
                    // same context schema name with different definitions cannot occur in multiple policies
                    if (axContextSchemas.getSchemasMap().get(key).equals(policyModel.getSchemas().get(key))) {
                        LOGGER.info("Same contextSchema - " + key.getId() + "is being used by multiple policies.");
                    } else {
                        errorMessage += "\n Same context schema - " + key.getId()
                            + " with different definitions used in different policies";
                    }
                }
            }
            for (AxArtifactKey key : policyModel.getEvents().getEventMap().keySet()) {
                if (axEvents.getEventMap().containsKey(key)) {
                    errorMessage += "\n Duplicate event found - " + key.getId();
                }
            }
            for (AxArtifactKey key : policyModel.getAlbums().getAlbumsMap().keySet()) {
                ...
                if (axContextAlbums.getAlbumsMap().containsKey(key)) {
                    if (axContextAlbums.getAlbumsMap().get(key).equals(policyModel.getAlbums().get(key))) {
                        LOGGER.info("Same contextAlbum - " + key.getId() + "is being used by multiple policies.");
                    } else {
                        errorMessage += "\n Same context album - " + key.getId()
                            + " with different definitions used in different policies";
                    }
                }
            }
            for (AxArtifactKey key : policyModel.getTasks().getTaskMap().keySet()) {
                if (axTasks.getTaskMap().containsKey(key)) {
                    errorMessage += "\n Duplicate task found - " + key.getId();
                }
            }
            for (AxArtifactKey key : policyModel.getPolicies().getPolicyMap().keySet()) {
                if (axPolicies.getPolicyMap().containsKey(key)) {
                    errorMessage += "\n Duplicate policy found - " + key.getId();
                }
            }

            if (!errorMessage.equals("")) {
                LOGGER.error("Policy model for " + policyIdent.getName() + ":" + policyIdent.getVersion()
                    + " is having duplicates. So this policy is not executed");
                apexParametersMap.remove(policyIdent);
            } else {
