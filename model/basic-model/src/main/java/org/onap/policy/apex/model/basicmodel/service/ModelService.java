 ...
 * down long call chains in modules such as the Apex engine and editor. The model service makes the
 * model and concept definitions available statically.
 *
 * <p>Note that the use of the model service means that only a single Apex model of a particular 
 * type may exist in Apex (particularly the engine) at any time. Of course the model in a JVM can be
 * changed at any time provided all users of the model are stopped and restrted in an orderly manner.
 *
