...
public class CliUtils {
    ...
    public static List<String> generateArgumentsForCliEditor(CommandLineParameters parameters,
        ...
        Method getter;
        Object argValue;
        for (Object k : optionVariableMap.keySet()) {
            String key = k.toString();
            try {
                pd = new PropertyDescriptor(optionVariableMap.getProperty(key), class1);
                getter = pd.getReadMethod();
                argValue = getter.invoke(parameters);

                if (argValue instanceof String && StringUtils.isNotEmpty(argValue.toString()) && !key.equals("o")) {
                    cliArgsList.add("-" + key);
