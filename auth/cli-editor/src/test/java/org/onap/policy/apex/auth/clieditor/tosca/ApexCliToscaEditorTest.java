...
public class ApexCliToscaEditorTest {
    ...
    public void testApexCliTosca_no_args() {
        ...
        try {
            new ApexCliToscaEditorMain(noArgs);
        } catch (Exception e) {
            assertEquals("Insufficient arguments provided.", e.getMessage());
        }
    }
    ...
    public void testApexCliTosca_missing_commandfile() {
        ...
        try {
            ...
            assertEquals("Insufficient arguments provided. Specify the command file or the policy model file. "
                + "Either one of these is required to generate the ToscaPolicy.", e.getMessage());
        }
    }
}
