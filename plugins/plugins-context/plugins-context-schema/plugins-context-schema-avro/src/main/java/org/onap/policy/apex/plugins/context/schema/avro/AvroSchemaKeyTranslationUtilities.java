...
public final class AvroSchemaKeyTranslationUtilities {
    ...
    private static String translateIllegalKey(final String key, final boolean revert) {
        if (revert) {
            return key.replaceAll(DOT_STRING_REPLACEMENT, DOT_STRING).replace(DASH_STRING_REPLACEMENT, DASH_STRING);
        } else {
            return key.replaceAll(DOT_STRING, DOT_STRING_REPLACEMENT).replace(DASH_STRING, DASH_STRING_REPLACEMENT);
        }
    }
}
