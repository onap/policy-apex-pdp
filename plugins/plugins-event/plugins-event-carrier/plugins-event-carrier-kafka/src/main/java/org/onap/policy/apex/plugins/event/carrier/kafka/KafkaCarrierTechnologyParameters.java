    ...
    private ValidationResult validateKafkaProperties() {
        ...
        for (int i = 0; i < kafkaProperties.length; i++) {
            ...
            if (kafkaProperties[i].length != 2) {
                result.addResult(label, Arrays.asList(kafkaProperties[i]), ValidationStatus.INVALID,
                        "kafka properties must be name-value pairs");
                continue;
            }

            BeanValidationResult result2 = new BeanValidationResult(label, Arrays.asList(kafkaProperties[i]));
