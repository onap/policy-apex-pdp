...
public class ApexGrpcProducer extends ApexPluginsEventProducer implements CdsProcessorListener {
    ...
    // The HTTP client that makes a GRPC call to get an input event for Apex
    ...
    public void init(final String producerName, final EventHandlerParameters producerParameters)
        ...
        client = new CdsProcessorGrpcClient(this, props);
    }
    ...
    public void sendEvent(final long executionId, final Properties executionProperties, final String eventName,
        ...
        try {
            executionServiceInput = coder.decode(coder.encode(event), ExecutionServiceInput.class);
        } catch (CoderException e) {
            throw new ApexEventRuntimeException(
                "Incoming Event cannot be converted to ExecutionServiceInput type for GRPC request = "
                    + e.getMessage());
        }
        try {
