...
public class JavascriptExecutor implements Runnable {
    ...
    private CountDownLatch threadControlLatch = new CountDownLatch(1);
    private StateMachineException executorException;
    ...
    public boolean execute(final Object executionContext) throws StateMachineException {
        ...
        try {
            executionQueue.put(executionContext);
        } catch (InterruptedException e) {
            LOGGER.debug("JavascriptExecutor {} interrupted on execution queue put", subjectKey.getId(), e);
            Thread.currentThread().interrupt();
        }
    ...
    public void run() {
        ...
        try {
            ...
            executorException = sme;
            threadControlLatch.countDown();
            return;
        }
        ...
        while (!executorThread.isInterrupted()) {
            ...
            try {
                contextObject = executionQueue.take();
            } catch (final InterruptedException e) {
                LOGGER.debug("JavascriptExecutor {} interrupted on execution queue take", subjectKey.getId(), e);
                Thread.currentThread().interrupt();
            }
            ...
            try {
                executionResult = executeScript(contextObject);
            } catch (StateMachineException sme) {
                LOGGER.debug("JavascriptExecutor {} execution failed", sme);
                executorException = sme;
            }

            try {
                resultQueue.put(executionResult);
            } catch (final InterruptedException e) {
                LOGGER.debug("JavascriptExecutor {} interrupted on execution result put", subjectKey.getId(), e);
                Thread.currentThread().interrupt();
            }
        }
        ...
        try {
            ...
            executorException = new StateMachineException(
    ...
    private void clearAndThrowExecutorException() throws StateMachineException {
        StateMachineException exceptionToThrow = executorException;
