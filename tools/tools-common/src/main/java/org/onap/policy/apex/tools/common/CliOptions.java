...
public final class CliOptions {
    ...
    public static final Option CONSOLE = Option.builder("c").longOpt("console")
                    .desc("application as console with input from standard in").build();

    /** A help option with "-h" and "--help". */
    public static final Option HELP = Option.builder("h").longOpt("help").desc("prints this help and usage screen")
                    .build();

    /** A version option with "-v" and "--version". */
    public static final Option VERSION = Option.builder("v").longOpt("version").desc("prints the application version")
                    .build();
    ...
    public static final Option PRINTSTACKTRACE = Option.builder().longOpt("print-stacktrace")
                    .desc("prints stack traces for any exception").build();
    ...
    public static final Option QUIET = Option.builder("q").longOpt("quiet")
                    .desc("application in quiet mode, no output at all").build();

    /** A no-warning option with "--no-warnings". */
    public static final Option NOWARNINGS = Option.builder().longOpt("no-warnings").desc("switch off all warnings")
                    .build();

    /** A no-error option with "--no-errors". */
    public static final Option NOERRORS = Option.builder().longOpt("no-errors").desc("switch off error messages")
                    .build();

    /** A no-progress option with "--no-progress". */
    public static final Option SHOWPROGRESS = Option.builder().longOpt("no-progress")
                    .desc("switch off progress information").build();

    /** A file-in option with "-f" and "--input-file". */
    public static final Option FILEIN = Option.builder("f").hasArg().argName("FILE").longOpt("input-file")
                    .desc("set the input file").build();
    ...
    public static final Option OVERWRITE = Option.builder("ow").required(false).longOpt("overwrite")
                    .desc("overwrite the output file if it exists. This option can only be used with the -"
                                    + FILEOUT.getOpt() + " option")
                    .build();
