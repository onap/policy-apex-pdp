...
public class ApexMonitoringRestResource {
     ...
     * @Path("modelupload/")
     *
     * @Consumes(MediaType.MULTIPART_FORM_DATA) public Response modelUpload(@FormDataParam("hostName") String
     * hostName, @FormDataParam("port") int port, @FormDataParam("file") InputStream
     * uploadedInputStream, @FormDataParam("file") FormDataContentDisposition
     * fileDetail, @FormDataParam("ignoreConflicts") boolean ignoreConflicts, @FormDataParam("forceUpdate") boolean
     * forceUpdate) { final EngineServiceFacade engineServiceFacade = new EngineServiceFacade(hostName, port);
     *
     * try { engineServiceFacade.init(); } catch (final ApexDeploymentException e) { String errorMessage =
     * "Error connecting to Apex Engine Service at " + hostName + ":" + port; LOGGER.warn(errorMessage + "<br>", e);
     * return Response.status(Response.Status.INTERNAL_SERVER_ERROR).entity(errorMessage + "\n" +
     * e.getMessage()).build(); }
     *
     * try { engineServiceFacade.deployModel(fileDetail.getFileName(), uploadedInputStream, ignoreConflicts); } catch
     * (final Exception e) { LOGGER.warn("Error updating model on engine service " +
     * engineServiceFacade.getKey().getID(), e); String errorMessage = "Error updating model on engine service " +
     * engineServiceFacade.getKey().getID(); LOGGER.warn(errorMessage + "<br>", e); return
