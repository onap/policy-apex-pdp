...
public class ContextAlbumImplTest {
    ...
    public void testNullsOnConstructor() {
        assertThatThrownBy(() -> new ContextAlbumImpl(null, null, null))
            .hasMessageContaining("Context album definition may not be null");

        assertThatThrownBy(() -> new ContextAlbumImpl(new AxContextAlbum(), null, null))
            .hasMessageContaining("Distributor may not be null");

        assertThatThrownBy(() -> new ContextAlbumImpl(new AxContextAlbum(), new JvmLocalDistributor(), null))
            .hasMessageContaining("Album map may not be null");

        assertThatThrownBy(() -> new ContextAlbumImpl(new AxContextAlbum(), new JvmLocalDistributor(),
                new LinkedHashMap<String, Object>()))
            .hasMessageContaining("Model for org.onap.policy.apex.model.contextmodel.concepts.AxContextSchemas "
    ...
    public void testAlbumInterface() throws ContextException {
        ...
        assertThatThrownBy(() -> album.get(null))
            .hasMessageContaining("album \"TestContextAlbum:0.0.1\" null keys are illegal on keys for get()");

        assertThatThrownBy(() -> album.put(null, null))
            .hasMessageContaining("album \"TestContextAlbum:0.0.1\" null keys are illegal on keys for put()");

        assertThatThrownBy(() -> album.put("KeyNull", null))
            .hasMessageContaining("album \"TestContextAlbum:0.0.1\" null values are illegal on key \"KeyNull\""
                    + " for put()");
        ...
        assertThatThrownBy(() -> albumRo.put("KeyReadOnly", "A value for a Read Only Album"))
            .hasMessageContaining("album \"TestContextAlbum:0.0.1\" put() not allowed on read only albums "
        ...
        assertThatThrownBy(() -> albumRo.putAll(putAllData))
            .hasMessageContaining("album \"TestContextAlbum:0.0.1\" putAll() not allowed on read only albums");

        assertThatThrownBy(() -> albumRo.remove("AllKey0"))
            .hasMessageContaining("album \"TestContextAlbum:0.0.1\" remove() not allowed "
                            + "on read only albums for key=\"AllKey0\"");

        assertThatThrownBy(() -> album.remove(null))
            .hasMessageContaining("null values are illegal on method parameter \"keyID\"");

        assertThatThrownBy(() -> albumRo.clear())
            .hasMessageContaining("album \"TestContextAlbum:0.0.1\" clear() not allowed on read only albums");
        ...
        assertThatThrownBy(() -> album.flush())
            .hasMessageContaining("map flush failed, supplied map is null");
        ...
        assertThatThrownBy(() -> album.putAll(putAllData))
            .hasMessageContaining("album \"TestContextAlbum:0.0.1\" null values are illegal on key "
                    + "\"AllKey3\" for put()");
    ...
    public void testCompareToEqualsHash() throws ContextException {
        ...
        assertThatThrownBy(() -> {
            ...
            .hasMessageContaining("Failed to set context value for key \"Key\" in album \"TestContextAlbum:0.0.1\": "
